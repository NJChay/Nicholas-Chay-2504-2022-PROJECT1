...
function factor(f::PolynomialSparseBI, prime::Int)::Vector{Tuple{PolynomialSparseBI,Int}}
    #Cantor Zassenhaus factorization
    ...
    ret_val = Tuple{PolynomialSparseBI,Int}[]
...
function expand_factorization(factorization::Vector{Tuple{PolynomialSparseBI,Int}})::PolynomialSparseBI
    length(factorization) == 1 && return first(factorization[1])^last(factorization[1])
    return *([first(tt)^last(tt) for tt in factorization]...)
end
...
function multiplicity(f::PolynomialSparseBI, g::PolynomialSparseBI, prime::Int)::Int
    degree(gcd(f, g, prime)) == 0 && return 0
    return 1 + multiplicity((f ÷ g)(prime), g, prime)
end
...
function dd_factor(f::PolynomialSparseBI, prime::Int)::Array{PolynomialSparseBI}
    x = PolynomialSparseBI(BTerm(BigInt(1),1))
    w = deepcopy(x)
    g = Array{PolynomialSparseBI}(undef,degree(f)) #Array of polynomials indexed by degree
    ...
    #edge case for final factor
    f != one(PolynomialSparseBI) && push!(g,f)
...
function dd_split(f::PolynomialSparseBI, d::Int, prime::Int)::Vector{PolynomialSparseBI}
    f = mod(f,prime)
    degree(f) == d && return [f]
    degree(f) == 0 && return []
    w = rand(PolynomialSparseBI, degree = d, monic = true)
    w = mod(w,prime)
    n_power = (prime^d-1) ÷ 2
    g = gcd(pow_mod(w,n_power,prime) - one(PolynomialSparseBI), f, prime)
    ḡ = (f ÷ g)(prime) # g\bar + [TAB]
    return vcat(dd_split(g, d, prime), dd_split(ḡ, d, prime) )
end