...
show(io::IO, p::PolynomialModP) = print(p.poly, " (mod ",p.prime,")")
...
function factor_test_polymod(;N::Int = 1, seed::Int = 0, primes::Vector{Int} = [5,17,19])
    Random.seed!(seed)
    for prime in primes
        print("\ndoing prime = $prime \t")
        for _ in 1:N
            ...
            @assert poly(pr)==poly(p)
        end
    end

    println("\nfactor_test_polymod - PASSED")
end